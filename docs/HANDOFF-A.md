...
| 08 | engram/server.py, engram/__main__.py, pi-extension/src/index.ts | done, LIVE VERIFIED with openai/gpt-4.1-mini (see HOOKS.md bottom): recall before first token, remember after each turn, cross-session recall used by the model | pytest test_server (stdio roundtrip), node --test mocked-pi hooks, tsc clean |
...
| merges | lane/b merged (7 commits) + wired: server.remember → p1.extract_claims / p3.route_claim / p2.promote, thinking via p1.think, FTS OR-query for recall, DAG encode fallback to salience rules. lane/c fully merged (encoder + tests, evolve + tests, README); DAG evolve node produces generations. lane/d merged (metrics, report, LongMemEval slice + driver + judge, component bench, DEMO.md, README). All four lanes in. | pytest 32 passed 1 xpassed; CUJ fixtures driven end to end through the server: session 2 recalls session 1, absence claim → one question, correction refutes the old claim, consolidate wires links |
| 14 | README pointers, final PR | pending | |

Done as integrator: activation decay in sqlite store (half-life ≈14 d at λ=0.05, immune kinds, dormant never deleted, revives on use), DAG expire node reads λ/threshold from the latest genome. Seven-beat live rehearsal with gpt-4.1-mini done (19:1x): user-only tagging, one claim per sentence, always-on USER/MEMORY injected ahead of recalled claims, CLI consolidate reads pending episodes; Wednesday opening applies both preferences unprompted; correction refutes; report tool called. Lane D defects 2/3/4 addressed (held claims stay unretrievable by design). Open items: forced /compact live check (A-6) and TUI question notify still unverified live. Interactive users must accept pi's project-trust prompt once (or pass -a). Tell lane B that rules.extract_claims promotes EVERY unmatched turn (assistant turns included) as a `fact` — memory inflates; suggest unmatched turns stay episodic-only. A-2/A-4 probes (lane D on the demo machine). Showcase 19:00 vs 20:00 unconfirmed.
