...
| merges | lane/b merged (7 commits) + wired: server.remember → p1.extract_claims / p3.route_claim / p2.promote, thinking via p1.think, FTS OR-query for recall, DAG encode fallback to salience rules. lane/c fully merged (encoder + tests, evolve + tests, README); DAG evolve node produces generations. lane/d pending | pytest 32 passed 1 xpassed; CUJ fixtures driven end to end through the server: session 2 recalls session 1, absence claim → one question, correction refutes the old claim, consolidate wires links |
