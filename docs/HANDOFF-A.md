...
| merges | lane/b merged (7 commits) + wired: server.remember → p1.extract_claims / p3.route_claim / p2.promote, thinking via p1.think, FTS OR-query for recall, DAG encode fallback to salience rules. lane/c merged (encoder only: engram/encode + prompts + recorded fixtures; NO tests/test_encode.py and NO engram/evolve yet — lane C still owes evolve + tests). lane/d pending | pytest 32 passed 1 xpassed; CUJ fixtures driven end to end through the server: session 2 recalls session 1, absence claim → one question, correction refutes the old claim, consolidate wires links |
| 14 | README pointers, final PR | pending | |

Open items: tell lane B that rules.extract_claims promotes EVERY unmatched turn (assistant turns included) as a `fact` — memory inflates; suggest unmatched turns stay episodic-only. A-2/A-4 probes (lane D on the demo machine). Showcase 19:00 vs 20:00 unconfirmed.
